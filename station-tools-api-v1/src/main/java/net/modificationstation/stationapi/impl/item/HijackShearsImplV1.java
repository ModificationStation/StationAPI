...
public class HijackShearsImplV1 {
    ...
    private static void hijackShearsEvent(ShearsOverrideEvent event) {
        event.overrideShears = !event.overrideShears && ((ToolLevel) event.itemStack.getType()).getEffectiveBlocks(event.itemStack).id().equals(Identifier.of("mineable/shears"));
    }
}
