...
public class HasCustomBlockItemFactoryImpl implements BlockItemFactoryProvider {
    ...
    /**
     * Processes {@link HasCustomBlockItemFactory.At} annotation if present via {@link BlockItemFactoryProvider} hook.
    ...
    public IntFunction<PlaceableTileEntity> getBlockItemFactory(BlockBase block, IntFunction<PlaceableTileEntity> currentFactory) {
        if (block instanceof HasCustomBlockItemFactory)
            currentFactory = ((HasCustomBlockItemFactory) block).getBlockItemFactory();
        Class<? extends BlockBase> blockClass = block.getClass();
        if (blockClass.isAnnotationPresent(HasCustomBlockItemFactory.At.class)) {
            ...
            try {
                ...
                currentFactory = (IntFunction<PlaceableTileEntity>) LambdaMetafactory.metafactory(
                        ...
                        lookup.findConstructor(blockClass.getAnnotation(HasCustomBlockItemFactory.At.class).value(), MethodType.methodType(void.class, int.class)),
