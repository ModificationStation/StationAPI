...
public enum Blocks {
    ...
    CUSTOM_MODEL_BLOCK("farlands_block", "farlands_block", id -> new ModdedModelBlock(id, Material.DIRT).setHardness(1).mineableBy(of("tools/shovels"), 0)),
    FREEZER("freezer", "freezer", id -> new BlockFreezer(id).setHardness(2.5F).setSounds(TemplateBlockBase.STONE_SOUNDS).mineableBy(of("tools/pickaxes"), 0)),
    ALTAR("altar", "altar", id -> new BlockAltar(id, Material.STONE).setHardness(3)),
    VARIATION_BLOCK("variation_block", "variationBlock", id -> new VariationBlock(id, Material.STONE).setHardness(.5F).mineableBy(of("tools/pickaxes"), 0).setSounds(BlockBase.STONE_SOUNDS).disableBlockItem());
