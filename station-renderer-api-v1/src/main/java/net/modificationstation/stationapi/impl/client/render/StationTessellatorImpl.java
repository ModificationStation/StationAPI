...
import net.modificationstation.stationapi.api.util.math.RotationAxis;
...
public class StationTessellatorImpl implements StationTessellator {
    ...
    public void quad(BakedQuad quad, float x, float y, float z, int colour0, int colour1, int colour2, int colour3, float normalX, float normalY, float normalZ, boolean spreadUV) {
        ...
        System.arraycopy(quad.vertexData(), 0, fastVertexData, 0, 32);
        ...
        if (spreadUV) {
            Direction facing = quad.face();
            Matrix4f texture = new Matrix4f();
            texture.m03((float) access.getXOffset()).m13((float) access.getYOffset()).m23((float) access.getZOffset());
            texture.invert();
            ...
            texture.transform(damageUV);
            damageUV.rotate(RotationAxis.POSITIVE_Y.rotationDegrees(180.0F));
            damageUV.rotate(RotationAxis.POSITIVE_X.rotationDegrees(-90.0F));
            damageUV.rotate(facing.getRotationQuaternion());
            fastVertexData[3] = Float.floatToRawIntBits(-damageUV.x());
            fastVertexData[4] = Float.floatToRawIntBits(-damageUV.y());
            ...
            damageUV.rotate(RotationAxis.POSITIVE_X.rotationDegrees(-90.0F));
            damageUV.rotate(facing.getRotationQuaternion());
            fastVertexData[11] = Float.floatToRawIntBits(-damageUV.x());
            fastVertexData[12] = Float.floatToRawIntBits(-damageUV.y());
            ...
            damageUV.rotate(RotationAxis.POSITIVE_X.rotationDegrees(-90.0F));
            damageUV.rotate(facing.getRotationQuaternion());
            fastVertexData[19] = Float.floatToRawIntBits(-damageUV.x());
            fastVertexData[20] = Float.floatToRawIntBits(-damageUV.y());
            ...
            damageUV.rotate(RotationAxis.POSITIVE_X.rotationDegrees(-90.0F));
            damageUV.rotate(facing.getRotationQuaternion());
            fastVertexData[27] = Float.floatToRawIntBits(-damageUV.x());
            fastVertexData[28] = Float.floatToRawIntBits(-damageUV.y());
        }
