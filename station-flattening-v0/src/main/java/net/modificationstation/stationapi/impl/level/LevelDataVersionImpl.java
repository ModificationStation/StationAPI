...
public final class LevelDataVersionImpl {
    ...
    private static void addDataVersions(LevelPropertiesEvent.Save event) {
        Map.Entry<String, ? extends AbstractTag> entry = Iterators.getOnlyElement(((CompoundTagAccessor) DataFixers.addDataVersions(NbtOps.INSTANCE, new CompoundTag())).stationapi$getData().entrySet().iterator());
