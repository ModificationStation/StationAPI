...
public class TextureFactoryOld implements net.modificationstation.stationapi.api.client.texture.TextureFactoryOld {

    @Getter
    private final String
            originalStationAtlasFormat = "/assets/station-render-api-v0/atlases/station.%s.png",
            copiedStationAtlasFormat = "/assets/station-render-api-v0/atlases/station.%s.%s.png";
    private final Map<String, String> fakedAtlases = new HashMap<>();
    private final Map<TextureRegistryOld, TreeMap<Integer, Integer>> spriteIDs = new HashMap<>();
    private TextureOld[] textures = new TextureOld[0];

    @Override
    public int addTexture(TextureRegistryOld type, String pathToImage) {
        TextureOld texture = new StaticTextureOld(type, pathToImage);
        try {
    ...
    public int addAnimatedTexture(TextureRegistryOld type, String pathToImage, int animationRate) {
        TextureOld texture = new AnimatedTextureOld(type, pathToImage, animationRate);
        try {
            setupTexture(((Minecraft) FabricLoader.getInstance().getGameInstance()).textureManager, texture);
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    private void setupTexture(TextureManager textureManager, TextureOld texture) throws IOException {
