...
public enum Items implements Supplier<@NotNull ItemBase> {

    // TODO: add the items that are not block items
    STONE("stone"),
    ...
    Items(final @NotNull String id) {
        this.id = of(id);
    }

    @Override
    public @NotNull ItemBase get() {
        return ItemRegistry.INSTANCE.get(id).orElseThrow();
    }
}
